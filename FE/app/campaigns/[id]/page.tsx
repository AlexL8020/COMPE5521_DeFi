...
export default function CampaignPage({ params }: { params: { id: string } }) {
  const [campaign, setCampaign] = useState(null); // State for campaign data
  const [loading, setLoading] = useState(true); // Loading state
  const [error, setError] = useState(null); // Error state

  // Fetch campaign data from MongoDB based on ID
  useEffect(() => {
    const fetchCampaign = async () => {
      try {
        const response = await fetch(`${API_CONFIG.BASE_URL}/api/v1/campaign/${params.id}`, {
          method: 'GET',
          headers: {
  ...
  if (error || !campaign) {
    return <div className="container py-8 text-center text-red-500">{error || 'Campaign not found'}</div>;
  }
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="flex items-center gap-2">
              <Avatar className="h-8 w-8">
                <AvatarImage src="/placeholder.svg?text=AJ" />
                <AvatarFallback>{campaign.creator?.charAt(0) || 'A'}</AvatarFallback>
              </Avatar>
              <span className="text-sm font-medium">{campaign.creator}</span>
            </div>
            <Badge variant="outline">{campaign.category}</Badge>
          ...
          <Tabs defaultValue="about">
            <TabsList className="mb-4">
              ...
              <TabsTrigger value="comments">Comments ({campaign.comments?.length || 0})</TabsTrigger>
            </TabsList>
        ...
        <div className="space-y-6">
          <div className="border rounded-lg p-6 bg-card">
            <div className="mb-4">
              <div className="flex justify-between mb-2">
                <span className="font-semibold">{progress.toFixed(2)} ETH raised</span>
                <span className="text-muted-foreground">of {fundingGoal} ETH goal</span>
              </div>
              <Progress value={percentFunded} className="h-2" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="border rounded-md p-3 text-center">
                <div className="text-2xl font-bold">{campaign.backers || 0}</div>
                <div className="text-xs text-muted-foreground">Backers</div>
              </div>
              <div className="border rounded-md p-3 text-center">
                <div className="text-2xl font-bold">{campaign.daysLeft || 0}</div>
                <div className="text-xs text-muted-foreground">Days Left</div>
              </div>
            </div>
          ...
          <div className="border rounded-lg p-6 bg-card">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <div className="font-medium">Funding Deadline</div>
                  <div className="text-sm text-muted-foreground">In {campaign.daysLeft || 0} days</div>
                </div>
              </div>
              <div className="flex gap-3">
                <div className="w-1 bg-muted rounded relative">
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    Estimated: {campaign.timeline || 'N/A'}
                  </div>
                </div>
              </div>
            </div>
